function [u_first_move,fcost,Ecount,exitflag,y_pred,hk,ck,xPx, Tconst] = get_control_input(y_set,yo,ypast,uo,upast,d,tp,ts,e,ho,Co,P,alpha)
    % e = 0;
    data = struct();
    data.upast = upast;
    data.ypast = ypast;
...
function [c,ceq] = nlcf(u,data)
    ...
    xt=[xo-y_set] ;  %     xt=[ck;hk]% xt=[xo-y_set]
       % xo=[Co;ho]   %      xo=[Co;ho]
